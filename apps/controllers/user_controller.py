...
def get_user_endpoint(nim: str, db: Session = Depends(get_db)):
...
def update_user_endpoint(nim: str, user_data: dict, db: Session = Depends(get_db)):
    return update_user(db, nim, user_data)

@router.delete("/{nim}", response_model=dict)
def delete_user_endpoint(nim: str, db: Session = Depends(get_db)):
