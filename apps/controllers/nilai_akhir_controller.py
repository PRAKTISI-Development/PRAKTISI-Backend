...
def create_nilai_akhir_endpoint(nilai_akhir_data: dict, db: Session = Depends(get_db)):
    return create_nilai_akhir(db, nilai_akhir_data)

@router.get("/{praktikan_nim}/{mata_kuliah_kode_matkul}", response_model=dict)
def get_nilai_akhir_endpoint(praktikan_nim: str, mata_kuliah_kode_matkul: str, db: Session = Depends(get_db)):
    return get_nilai_akhir(db, praktikan_nim, mata_kuliah_kode_matkul)

@router.get("/", response_model=list)
def get_nilai_akhirs_endpoint(db: Session = Depends(get_db)):
    return get_nilai_akhirs(db)

@router.put("/{praktikan_nim}/{mata_kuliah_kode_matkul}", response_model=dict)
def update_nilai_akhir_endpoint(praktikan_nim: str, mata_kuliah_kode_matkul: str, nilai_akhir_data: dict, db: Session = Depends(get_db)):
...
def delete_nilai_akhir_endpoint(praktikan_nim: str, mata_kuliah_kode_matkul: str, db: Session = Depends(get_db)):
