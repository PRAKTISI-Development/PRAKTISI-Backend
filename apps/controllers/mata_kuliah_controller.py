...
def get_mata_kuliah_by_code(kode_matkul: str, db: Session = Depends(get_db)):
    return mata_kuliah_service.get_mata_kuliah_by_code(db, kode_matkul)

@router.post("/", response_model=MataKuliah)
def create_mata_kuliah(mata_kuliah_data: dict, db: Session = Depends(get_db)):
    return mata_kuliah_service.create_mata_kuliah(db, mata_kuliah_data)

@router.put("/{kode_matkul}", response_model=MataKuliah)
def update_mata_kuliah(kode_matkul: str, mata_kuliah_data: dict, db: Session = Depends(get_db)):
...
def delete_mata_kuliah(kode_matkul: str, db: Session = Depends(get_db)):
